...
public class DeepSeekDialogueManager1 : MonoBehaviour
{
    [Header("TTS API 配置")]
...
//角色思考文本动画
    ...
    private IEnumerator CallTTSService(string botMessage)
    {
        ...
        List<IMultipartFormSection> formData = new List<IMultipartFormSection>
        {
            new MultipartFormFileSection(
                "ref_wav_path",
                File.ReadAllBytes(Path.Combine(Application.streamingAssetsPath, "ref.wav")),
                "ref.wav",
                "audio/wav"
            ),
        ...
        using (UnityWebRequest www = new UnityWebRequest(ttsApiUrl, "GET"))
        {
            ...
            else
            {
                ...
                if (www.result == UnityWebRequest.Result.Success)
                {
                    ...
                    Debug.Log($"TTS 文件已生成，路径: {tempAudioPath}");
    ...
    private IEnumerator WaitForFileWritten(string path)
    {
        ...
        while (unchangedCount < 8) // 连8次大小不变视为写入完成
        {
