...
public class XunfeiSpeechToText : STT
{
    ...
    /// 语言
    /// </summary>
    [SerializeField] private string m_Language = "en_us";
    /// <summary>
    /// 应用领域
    /// </summary>
    ...
    private string GetUrl()
    {
        ...
        string date = DateTime.UtcNow.ToString("r"); // 使用 UTC
        string signature_origin = $"host: {host}\ndate: {date}\nGET /v2/iat HTTP/1.1";

        string signature_sha = Convert.ToBase64String(
            new HMACSHA256(Encoding.UTF8.GetBytes(m_XunfeiSettings.m_APISecret))
            .ComputeHash(Encoding.UTF8.GetBytes(signature_origin))
        );

        string authorization_origin = $"api_key=\"{m_XunfeiSettings.m_APIKey}\",algorithm=\"hmac-sha256\",headers=\"host date request-line\",signature=\"{signature_sha}\"";
        string authorization = Convert.ToBase64String(Encoding.UTF8.GetBytes(authorization_origin));

        string encodedAuth = Uri.EscapeDataString(authorization);
        string encodedDate = Uri.EscapeDataString(date);

        Debug.Log("C#鉴权后URL：" + $"wss://{host}/v2/iat?authorization={encodedAuth}&date={encodedDate}&host={host}");
